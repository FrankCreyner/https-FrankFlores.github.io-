function calcularepFcn() {
    ...
    */
    ...
    Vtapas = Math.PI/12*(Math.pow(diametro + 2*espesortapareal,3) - Math.pow(diametro,3));
        ...
        R=diametro/2;
        theta=Math.acos((R-nivel)/R);
        Vliquido=Math.pow(R,2)*longitud*(theta-Math.sin(theta)*Math.cos(theta));
        document.querySelector('#volumenliquido').value = Vliquido.toFixed(0);

        pesoliquido=Vliquido*rho;
        document.querySelector('#pesoliquido').value = pesoliquido.toFixed(0);

        pesoequipo = (Vtapas + Vcuerpo)*rhoMaterial;
        document.querySelector('#pesoequipo').value = pesoequipo.toFixed(0);

        pesoequipolleno=pesoliquido+pesoequipo;
